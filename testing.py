...
def get_earnings_estimates(ticker):
    try:
        ...
        current_year = earnings.iloc[2]['avg']  # Current year (0y) average estimate
        next_year = earnings.iloc[3]['avg']     # Next year (+1y) average estimate
...
def get_revenue_estimates(ticker):
    try:
        ...
        current_year = revenue.iloc[2]['avg']  # Current year (0y) average estimate
        next_year = revenue.iloc[3]['avg']     # Next year (+1y) average estimate
        return pd.Series([current_year, next_year])
    except:
        return pd.Series([None, None])
